...
// For starting another FlashSlicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance)
{
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, FlashSlicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
	{
		...
		//bin_path = bin_path.parent_path() / "FlashSlicer";
